...
| `--overwrite` | Updates the existing user with that username. Without it the command does not check, and running it twice for one username creates a second user — after which that username can no longer log in |
