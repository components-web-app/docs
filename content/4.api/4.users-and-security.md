...
`route_security` accepts any number of patterns. Each `route` value supports a `*` wildcard (matched as a SQL `LIKE` pattern against `Route.path`). The `security` value is any Symfony expression-language security expression.
...
Controls who can see un-routed pages and page data in collection endpoints:
