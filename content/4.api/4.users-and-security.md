...
1. Client sends credentials to `POST /login` (`/_api/login` in the template, where the bundle's routes are mounted under `/_api`)
...
| `newEmailAddress` | `User:input`, `User:output` | Pending email change. Set it to `null` to [cancel the change](#cancelling-a-pending-change) |
...
There is no working Flex recipe for the bundle (see [Bundle Setup](/api/bundle-setup#installation)), so start from the template app's `security.yaml`. This is its shape:
...
- The request's `Origin` header (or `Referer` when there's no `Origin`) is used only when it matches one of `allowed_origins`.
- Otherwise the link uses `default_origin`.
- With neither, the link is refused and the email isn't sent. See [When a link is refused](#when-a-link-is-refused).
...
::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**Upgrading to 2.0.0-alpha.4 or later:** set `email_links.default_origin` (or `allowed_origins`). Without it, every email that carries a link is refused, and password reset and resend-verification requests return 400.
::

::callout{icon="i-heroicons-information-circle"}
The bundle anchors each `allowed_origins` pattern to the whole origin and matches it case-insensitively. Nelmio's `origin_regex` does not anchor, so check a `CORS_ALLOW_ORIGIN` value before you copy it here. The origin being matched has no path and no trailing slash, and a default port is dropped (`https://app.example.com`, not `https://app.example.com:443/`). Only `http` and `https` origins are accepted.
::

A path requested through `redirect_path_query` must be a plain relative path: it starts with a single `/` and has no `//`, backslash, space or control character. Anything else falls back to the flow's `default_redirect_path`. A `default_redirect_path` that is a full URL is used as it is and needs no origin.

### When a link is refused

What happens depends on whether the email is the request's whole job:

| Email | When the link is refused |
|-------|--------------------------|
...
| 400 | The link was refused (see [When a link is refused](#when-a-link-is-refused)) |
...
Independently of `route_security`, a route whose go-live date has not arrived is hidden in both places from anyone who fails `publishable.permission`. See [Scheduling and Taking Routes Offline](/api/dynamic-pages#scheduling-and-taking-routes-offline).
...
- **A page that nothing routes to is private.** No Route on the page and none on any page below it means its Page, PageData and components are readable only by users who pass `routable_security`.

Routes themselves are always held to this rule. For pages, page data and components it is enforced when [`routable_security`](#routable_security) is set, as it is in the template app. Reachability is resolved per request by walking the page hierarchy; there is no join table to rebuild and no command to run.

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**Upgrading:** components on a page that nothing routes to used to be publicly readable even though the page was not. They are now private too. If you deliberately served components from an unrouted page, give that page a Route or place it under a routed one. Components that are not placed on any page are unchanged, and an application that does not set `routable_security` sees no change at all.
::
...
- Users who **fail** it can fetch any page or page data that is [publicly reachable](#public-access-follows-routes), and any template Page that reachable page data renders through. Collections (`GET /_/pages`, page-data collections) list only records with their **own** live Route, so an unrouted parent is readable by IRI but not listed.
- Anything else is denied: 401 for an anonymous visitor, 403 for a signed-in user.
...
1. **Page check** — is the component placed (directly, through a layout, or nested inside another component) on a page that is [publicly reachable](#public-access-follows-routes)? If yes, access is granted.
2. **PageData check** — is the component referenced as a property on a `PageData` resource that is reachable? If yes, access is granted. The property can be typed as the component's own class or any parent class, such as `AbstractComponent`.
3. **Template check** — is the component placed in a page template used by reachable page data? If yes, access is granted.
4. If at least one check found a location but none of them is reachable, access falls back to `routable_security`: users who pass it can read the component, everyone else is denied (401 when anonymous).

This means components placed exclusively on admin, unrouted or not-yet-live pages are hidden from anonymous API clients.

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
A component with **no** location at all — not in any component group, not referenced by page data, not in a template — makes all three checks abstain, and the voter then **grants** access. An unplaced or orphaned component is readable by anyone who knows its IRI (the [orphaned resource report](/api/console-commands#orphaned-resource-report) lists them). Use `#[Silverback\Publishable]` or your own security expression if a component must stay private before it is placed.
::

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**Upgrading:** up to `2.0.0-alpha.5`, a page data property typed as a parent class (`ManyToOne(targetEntity: AbstractComponent::class)`, or a class the component extends) wasn't seen as a location. A component held only that way was readable by anyone. It now follows the page data: public only while the page data is reachable from a live route, and otherwise limited to users who pass `routable_security`. Editing it also purges the page data from the cache and publishes it to Mercure, which it didn't before.
::
