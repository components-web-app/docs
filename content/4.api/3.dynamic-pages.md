...
The bundle's `OrSearchFilter` is deprecated, and the bundle no longer uses it. If your own entities use it, replace it with a `search` parameter as above. Until then it still works, and it ignores a value that isn't an integer for an integer field (logging the notice "Invalid filter ignored") instead of returning a `500`.
