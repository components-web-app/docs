...
A depth also includes its page's layout, with the layout's component groups, their positions and their components. A layout that a parent and a child page share is listed once, at the shallowest depth that uses it. A template page shared across depths is still listed at each depth. `GET /_/layouts/{id}` is unchanged and still returns bare group IRIs. The layout's groups are only expanded in the manifest.

::callout{icon="i-heroicons-information-circle"}
Bundle versions before [api-components-bundle#307](https://github.com/components-web-app/api-components-bundle/pull/307) left the layout's groups out of the manifest. The module found them from the layout's own response, one round trip later.
::
