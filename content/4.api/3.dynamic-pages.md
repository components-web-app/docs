...
::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**Upgrading: a custom route generator must implement `generatePath()`.** `RouteGeneratorInterface` gains `generatePath(RoutableInterface $object): string` ([api-components-bundle#335](https://github.com/components-web-app/api-components-bundle/pull/335), in the release after 2.0.0-alpha.4). It returns the path `create()` would give the record, before any `-1` suffix for a conflict. The fixture builder uses it to find existing page data on an [appended load](/api/data-fixtures#appending-to-an-existing-site). If your application replaces or decorates the `RouteGeneratorInterface` service with a class of its own, add the method. Without it, PHP refuses to load the class, with a fatal error for the missing interface method. A decorator can pass the call on to the inner generator.
::
