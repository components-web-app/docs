...
A draft copy is never listed; it's judged by its published version. A component held by a page data property counts as used even when the property is typed as a parent class, such as `AbstractComponent`. The report always counted it; up to `2.0.0-alpha.5` the old `clean-orphaned` and the `/usage` count did not, so that command could delete a component the report left alone.
