...
When `orphaned_resources.notify.recipients` is set, the command emails those addresses when its scan differs from the report it replaces. The recipients, admin page path and subject are in [Configuration](/api/configuration#full-configuration):
...
The report has changed when any of the three lists holds different IRIs. Order doesn't matter. The first report ever stored counts as a change only if it has orphans. The email gives the count of each kind, the IRIs that are new since the last scan, how many are no longer orphaned, and a **Review Orphaned Resources** button that opens the admin page.

::callout{icon="i-heroicons-information-circle"}
Only `scan-orphaned` sends the email. `POST /_/orphaned_resources/scan` and the refresh after [`POST /_/orphaned_resources/delete`](#deleting-orphans) never do, but they still replace the stored report. The next run of the command compares against that newer report, so a change an admin has already scanned for doesn't send an email.
::

`--notify` is on by default. Pass `--no-notify` to scan and store the report without emailing. That run still replaces the report, so its changes aren't emailed later either.
