...
::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
The generated SQL names the tables `abstract_component` and `component_group`, without the bundle's `table_prefix`. With the default prefix the real tables are `_acb_abstract_component` and `_acb_component_group`, so edit the migration before you run it. It also writes the class to `src/Migrations/` in the `App\Migrations` namespace, while the template's `doctrine_migrations.yaml` only reads `migrations/` (namespace `DoctrineMigrations`). Move the file there and change its namespace, or `doctrine:migrations:migrate` won't find it.
::
