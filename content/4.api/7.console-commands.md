...
The command prompts for the class name only — properties are never prompted for, so pass them with `--properties`:
...
Properties are defined as `name:type` pairs, with the `?` prefix for nullable types.

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
`--properties` is an array option, so repeat it once per property. Passing several space-separated values after a single `--properties` makes Symfony read all but the first as extra arguments and the command aborts with "Too many arguments". Run the bare command with no `--properties` at all and you get an entity with no properties and no warning.
::
...
- A `nuxt.config.ts` starter snippet listing the property names under `cwa.pageData.ConferenceData`
...
::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
The printed `nuxt.config.ts` snippet emits `properties` as a plain array of names, but the module's config schema expects a map of property name → human-readable label (see [nuxt.config](/nuxt-module/configuration/nuxt-config)). Convert it by hand: `properties: { headline: 'Headline', body: 'Body' }`.
::
