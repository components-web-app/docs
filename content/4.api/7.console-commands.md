...
It takes no arguments and always exits with `0`. It runs the same scan as `POST /_/orphaned_resources/scan`, but always during the command, even when you route the scan message to a Messenger worker. That makes it suitable for a cron job or a deploy step that keeps the report fresh.
