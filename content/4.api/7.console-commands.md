...
| `--overwrite` | Update the user if they already exist. Without it the command does not check, so repeating a username creates a duplicate user that can no longer log in |
...
# Expire by email instead of username (--field takes the entity property name)
...
::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
The command writes the migration class to `src/Migrations/` in the `App\Migrations` namespace, while the template's `doctrine_migrations.yaml` only reads `migrations/` (namespace `DoctrineMigrations`). Move the file there and change its namespace, or `doctrine:migrations:migrate` won't find it.
::
...
::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
This command writes the migration for you — do **not** run `make:migration` afterwards, or you will get a second, unrelated diff migration. Review the generated SQL in `src/Migrations/RenameComponent…php`, then apply it with `php bin/console doctrine:migrations:migrate`.
::
