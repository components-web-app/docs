...
**Use bare addresses.** Up to bundle `2.0.0-alpha.6`, each recipient must be a plain address such as `ops@example.com`. The `Name <address>` form, such as `My Website <website@example.com>`, is refused ([api-components-bundle#361](https://github.com/components-web-app/api-components-bundle/issues/361); the fix is in [PR #363](https://github.com/components-web-app/api-components-bundle/pull/363) and not yet released):

- Written in the config file, it fails the container build with "The orphaned resources notification recipients … must be email addresses."
- Supplied by an environment variable, it passes the config check. The scan then logs that the recipient is not a valid email address, sends nothing to anyone, and prints the "could not be sent" line. The last alert doesn't move, so every run tries again.

Check your bundle's CHANGELOG for the release that includes #363 before relying on the `Name <address>` form.
::

The [components-web-app template](/deployment/kubernetes#daily-orphan-scan) schedules this command on production, daily at 03:00 London time, with `recipients: '%env(string:default::MAILER_EMAIL)%'`, so the alert goes to the site's own email address and an empty `MAILER_EMAIL` sends nothing. Without `string:`, an empty value becomes `null` and the command fails with a `TypeError` ([api-components-bundle#365](https://github.com/components-web-app/api-components-bundle/issues/365)). The template's `api/.env` default for `MAILER_EMAIL` is in the `Name <address>` form, so set a bare address in production until the fix above is released.
