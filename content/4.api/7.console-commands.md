...
The [components-web-app template](/deployment/kubernetes#daily-orphan-scan) schedules this command on production, daily at 03:00 London time, with `recipients: '%env(default::MAILER_EMAIL)%'`, so the alert goes to the site's own email address. The template's `api/.env` default for `MAILER_EMAIL` is in the `Name <address>` form, so set a bare address in production until the fix above is released.
