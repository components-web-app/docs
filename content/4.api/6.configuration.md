...
The required fields — omit any of them and the container fails to compile — are `website_name`, `user.class_name`, `publishable.permission`, the four `user.email_verification` booleans (`default_value`, `verify_on_register`, `verify_on_change`, `deny_unverified_login`), and `refresh_token.handler_id` / `cookie_name` / `ttl` / `database_user_provider`. The `publishable` and `email_verification` nodes are always processed even when you leave them out of your YAML, so their required children can't be skipped.
