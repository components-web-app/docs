...
To use the component, add it to the target group's `allowedComponents`: its FQCN in fixtures and the REST API, or its component name in the front end's `allowed-components` prop. A group whose `allowedComponents` is `null` still allows all *non-flagged* types — but a flagged type must always be named explicitly.
...
The flag is matched against the un-prefixed `title` key (the class short name), and it is **absent** rather than `false` on unflagged types — which is how the Nuxt module reads it.