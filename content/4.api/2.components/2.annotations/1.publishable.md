...
`OrSearchFilter` is now deprecated. The bundle's own collections take a single `search` parameter instead, and so should yours; see [Dynamic Pages → Searching and Sorting](/api/dynamic-pages#searching-and-sorting).
::
