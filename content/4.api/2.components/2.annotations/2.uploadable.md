...
The group is always `{ShortName}:published`. If you set custom `validationGroups` on `#[Silverback\Publishable]`, those replace that group when publishing, so `requiredOnPublish` is not checked.
