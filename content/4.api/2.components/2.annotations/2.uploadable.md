...
| `#[Assert\File(maxSize: ...)]` on the field | `20M` |
| `memory_limit` (PHP ini) | `512M` |
| Largest image accepted | 40 megapixels (about 470 MB to thumbnail) |
| Admin uploads resized in the browser ([`cwa.upload.image`](/nuxt-module/configuration/nuxt-config#image-uploads), a module default) | over 2560 px on the longest edge or over 20 megapixels |
...
#[Assert\When( // [!code focus]
    expression: 'value !== null && value.getMimeType() !== "image/svg+xml"', // [!code focus]
    constraints: [ // [!code focus]
        new Assert\Image( // [!code focus]
            maxPixels: 40_000_000, // [!code focus]
            maxPixelsMessage: 'This image is too large ({{ pixels }} pixels). Please resize it to at most {{ max_pixels }} pixels and upload it again.', // [!code focus]
        ), // [!code focus]
    ], // [!code focus]
)] // [!code focus]
public ?File $file = null;
...
If you raise `memory_limit`, raise the php pod's memory limit with it. See [Kubernetes → Resource Requests and Limits](/deployment/kubernetes#resource-requests-and-limits).

### Browser-side resizing

The Nuxt module resizes large JPEG, PNG and WebP images in the browser before an admin upload sends them. By default, anything over 2560 px on its longest edge or over 20 megapixels is resized to fit both. A 2560 px image is at most 6.6 megapixels, well under the 40-megapixel limit, so a resized photo needs about 77 MB to thumbnail instead of hundreds. See [useCwaResourceUpload](/nuxt-module/composables/admin-manager/use-cwa-resource-upload#large-images-are-resized-before-upload) for the options.
