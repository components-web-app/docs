...
::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**Long-running runtimes (FrankenPHP worker mode, RoadRunner):** use `2.0.0-alpha.2` or later, the first releases to include [PR #209](https://github.com/components-web-app/api-components-bundle/pull/209). `1.1.11` and `2.0.0-alpha.1` don't include it. Before it, clearing a file (`{ "file": null }`) was remembered by the worker after the request ended, so later publishes served by the same worker could delete their own files.
::
