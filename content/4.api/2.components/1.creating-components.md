...
| `uiClassNames` | `string[]\|null` | CSS class name(s) chosen by the admin in the CMS — one array entry per selected style |
...
The Nuxt module resolves `CwaComponentTitle` from the resource's `uiComponent`, falling back to its `@type` when `uiComponent` is empty. If a matching Vue component doesn't exist, it renders a warning in its place: `The component '…' for resource '…' cannot be resolved`.
...
The `Title:published` validation group is added by the bundle only when a resource is `#[Silverback\Publishable]` — use it for constraints that should hold on the published version but not block saving a draft. See [Publishable](/api/components/annotations/publishable).
...
```json
{
    ...
    "_metadata": {
        "publishable": {
            "published": true,
            "publishedAt": "2024-01-15T10:30:00.000+00:00",
            "locationCount": 1
        }
    }
}
...
## Restricting Where a Component Can Be Placed

By default, a component can be placed in any component group. To keep one out of the general add-component dialog, add the `#[Silverback\ExplicitAllowOnly]` attribute to the entity class — it can then only be added to groups that name it explicitly in their `allowedComponents`:
...
See [Explicit Allow Only](/api/components/annotations/explicit-allow-only) for the full reference.
