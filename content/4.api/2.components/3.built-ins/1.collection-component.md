...
The `collection` property in the response is a full Hydra collection. API Platform 4 omits the `hydra:` prefix on its keys by default (`member`, `totalItems`, `view`), which is what the Nuxt module reads:
...
```json
{
    ...
    "collection": {
        ...
        "@type": "Collection",
        "member": [ /* resource objects */ ],
        "totalItems": 24,
        "view": {
            "@type": "PartialCollectionView",
            "first": "/page_data/blog_articles?page=1",
            "last": "/page_data/blog_articles?page=4",
            "next": "/page_data/blog_articles?page=2"
        }
    }
}
```
...
Filters in `defaultQueryParameters` or the request only ever **narrow** the proxied collection. Drafts, scheduled routes and anything else the caller may not read stay out whatever filters are applied. Earlier releases had a bug where a search filter could bypass those checks, so a filtered collection may now return fewer items than before; see [Publishable → Filtered Collections](/api/components/annotations/publishable#filtered-collections), which also covers writing your own filter safely.

## On the Front-End

Use `useCwaComponent` with the `withCollection()` plugin in the Vue component. It unwraps `collection.member`, exposes `totalPages`, and provides `goToNextPage` / `goToPreviousPage` helpers — see [Collections & Pagination](/nuxt-module/component-helpers/collections-and-pagination) for the full reference.
