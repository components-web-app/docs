...
Access form state via `$cwa.forms.getForm(iri)` and per-field errors via `$cwa.forms.getFormViewErrors(iri, fieldName)`. The Nuxt module provides composables for binding form fields and handling the submission lifecycle — see the Authentication and Creating Components pages for usage patterns.
