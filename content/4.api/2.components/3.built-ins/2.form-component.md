...
A `PATCH` only validates: it never dispatches `FormSuccessEvent`, even when every field is valid. Only a `POST` or `PUT` submission runs the success handlers, so a listener that registers a user or sends an email won't fire while someone is still filling in the form. Bundle versions before [#264](https://github.com/components-web-app/api-components-bundle/pull/264) did fire it on a valid `PATCH`.
::
