...
Extend the bundle's `Silverback\ApiComponentsBundle\Form\AbstractType` (or implement its `FormTypeInterface`) rather than Symfony's. That interface is what tags the type as a CWA form type; a plain Symfony type fails the `formType` validation in Step 2:
...
The `formType` field is validated by the `FormTypeClass` constraint — it must be a form type service implementing `Silverback\ApiComponentsBundle\Form\FormTypeInterface` (autoconfiguration tags it).
...
A `PATCH` only validates: it never dispatches `FormSuccessEvent`, even when every field is valid. Only a `POST` submission runs the success handlers, so a listener that registers a user or sends an email won't fire while someone is still filling in the form. Bundle versions before [#264](https://github.com/components-web-app/api-components-bundle/pull/264) did fire it on a valid `PATCH`.
::
...
A `POST` is a full submit: any field the request leaves out is treated as empty, so a required field that's missing fails validation. A `PATCH` is partial and only checks the fields it sends. There is no `PUT` on `/submit`.

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**Changed in [api-components-bundle#280](https://github.com/components-web-app/api-components-bundle/pull/280).** `POST` used to be a partial submit. On a form without a `data_class`, a required field the request left out was never validated, so the submission succeeded. A client that deliberately posts only some of a form's fields now gets a **422**, with a violation for each missing required field.
::
