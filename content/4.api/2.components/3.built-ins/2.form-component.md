...
A **valid** `PATCH` also dispatches `FormSuccessEvent`. The bundle runs the success handlers for any valid request to `/submit`, whatever the method. A listener that should only act on the final submission has to check the request method itself.
::
