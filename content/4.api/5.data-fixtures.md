...
Returns a named `Route` once routes exist: inside an [`afterRoutes()`](#afterroutes) callback, or after `flush()`. Throws `LogicException` if called before routes have been created. Use to retrieve a route and assign it to a `NavigationLink` component.
...
| `pageData()` with `->withoutRoute()`, no `route` | No Route created |
...
- **`name`** defaults to the path with its slashes turned into hyphens: `/about` becomes `about`, and `/blog/old-post` becomes `blog-old-post`. Pass `name:` to choose it yourself.
...
The builder checks the redirect's **path** but not its **name**. If the name is already used by a different route, such as a page whose `routeName` is `about` when you redirect `/about`, saving fails on the route name's unique constraint. Pass a different `name:`.
::
...
::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**The generated class is always `App\DataFixtures\GeneratedScaffold`,** whatever file name you pass. For any other file name, or a directory outside `src/DataFixtures`, rename the class or change the namespace to match, or Symfony can't load it.
::
...
- **Routes**: every route by path and name, scheduled and offline routes through `liveAt()`, routeless page data through `withoutRoute()`, and redirects through `redirect()`.
...
::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**The scaffold holds content, not users.** Keep your [users fixture](#seeding-users), because the purge removes users too. And move the hand-written scaffold that the capture replaces out of the fixtures directory, or it loads as well.
::
