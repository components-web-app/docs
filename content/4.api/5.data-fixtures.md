...
**A `name:` you pass is not suffixed.** If a route created in the same load already has it, `flush()` throws a `LogicException` (`The redirect "…" cannot be named "…"`). If a saved route has it, the redirect isn't created and the summary counts it as skipped (`name in use`).
::
