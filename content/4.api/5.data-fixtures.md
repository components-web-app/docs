...
`AbstractCwaScaffold` implements `FixtureInterface`. Its `load()` method injects the `ObjectManager`, calls your `build()`, then calls `flush()` again. `CwaFixtureBuilder` is injected automatically via Symfony's service container — register your fixture class as a service and tag it with `doctrine.fixture.orm`.
