...
When seeding components that have HTML body fields (e.g. a rich-text `HtmlContent` entity), you need structured placeholder HTML rather than a raw lorem ipsum string. The CWA starter app ships a `CwaPlaceholderProvider` utility class at `src/PlaceholderProvider/CwaPlaceholderProvider.php` that generates CWA-themed HTML for fixtures. Copy it into your own app and register it as a service:
...
class AppFixtures extends AbstractCwaScaffold
{
    public function __construct(private readonly CwaPlaceholderProvider $placeholder)
    {
    }
    ...
    public function build(CwaFixtureBuilder $cwa): void
    {
        ...
        $content->html = $this->placeholder->generate([
            ...
            'includeCode'     => false,
            'includeLinks'    => true,
            'format'          => CwaPlaceholderProvider::FORMAT_HTML,
        ]);
...
```php
CwaPlaceholderProvider::LENGTH_SHORT    // 1–2 sentences per paragraph
CwaPlaceholderProvider::LENGTH_MEDIUM   // 3–4 sentences (default)
CwaPlaceholderProvider::LENGTH_LONG     // 5–7 sentences

CwaPlaceholderProvider::FORMAT_HTML      // returns HTML tags
CwaPlaceholderProvider::FORMAT_PLAINTEXT // returns plain text
```
...
You can also call `setOptions()` once at construction to set defaults for all subsequent `generate()` calls:

```php
$this->placeholder->setOptions(['paragraphLength' => CwaPlaceholderProvider::LENGTH_SHORT]);
$shortHtml  = $this->placeholder->generate();
$shortHtml2 = $this->placeholder->generate(); // same defaults
```
