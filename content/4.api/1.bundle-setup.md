...
Each release's changes are in the bundle's [`CHANGELOG.md`](https://github.com/components-web-app/api-components-bundle/blob/main/CHANGELOG.md), from `2.0.0-alpha.2` on, and the same notes are on its [GitHub releases](https://github.com/components-web-app/api-components-bundle/releases). Changes not yet released are listed under **Unreleased**. Read them before you upgrade, because breaking changes have their own heading.
...
::callout{icon="i-heroicons-information-circle"}
The bundle turns on API Platform's `use_symfony_listeners` for you, because its listeners and actions depend on it. Don't set it to `false` in your own `api_platform.yaml`: your value overrides the bundle's, and requests such as `GET /_api/me` then fail with a 500.
::
