...
The bundle's Flex recipe is published under the package's former name, `silverbackis/api-components-bundle`, so it does not run for `components-web-app/api-components-bundle` — and it is out of date anyway (its routing file points at `routing/all.xml`, which no longer exists). Create these files yourself; the CWA template app (`components-web-app`) has a working copy of each under `api/`:
...
- `config/routes/silverback_api_components.yaml` — imports `@SilverbackApiComponentsBundle/Resources/config/routing/all.php`, with the same `/_api` prefix as API Platform
