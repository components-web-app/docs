...
- [Symfony Mercure bundle](https://symfony.com/doc/current/mercure.html){target="_blank"} (`symfony/mercure-bundle`), installed by your app (see below)
...
```bash
composer require components-web-app/api-components-bundle symfony/mercure-bundle
```

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**Install `symfony/mercure-bundle` yourself.** The bundle decorates Mercure's default hub, but it currently lists Mercure only as a development dependency ([api-components-bundle#270](https://github.com/components-web-app/api-components-bundle/issues/270)), so Composer won't install it for you. Without it, the container won't compile.
::
