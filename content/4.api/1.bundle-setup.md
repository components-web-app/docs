...
The bundle works with API Platform 4.4 and 5. One difference is visible to clients: on API Platform 5, a request body with the wrong type for a property that has validation constraints returns a `422` with violations, where 4.4 returns a `400`. On both versions, `GET /me` returns `@context: /contexts/User`.
