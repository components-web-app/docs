...
The bundle maps some of its own exceptions to HTTP statuses through `api_platform.exception_to_status`. API Platform's default mappings apply alongside the bundle's:

| Exception | Status |
|-----------|--------|
| A Symfony Serializer exception, for example a relation given an IRI that doesn't resolve | `400` |
| API Platform's `InvalidArgumentException` | `400` |
| Doctrine's `OptimisticLockException` | `409` |
| Doctrine DBAL's `UniqueConstraintViolationException` | `422` on API Platform 5 only. API Platform 4.4 has no default for it, so it stays a `500` |

The bundle's own entries include a failed cache purge or flush (`HttpCachePurgeFailedException`, `HttpCacheFlushFailedException`), which `POST /_/rendered_html/purge` and `POST /_/http_cache/purge` return as `502`. A purge that fails after an ordinary write is only logged; see [Cache Tags and Purging](/api/configuration#cache-tags-and-purging).

An entry in your own `exception_to_status` config takes precedence over both.

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**Upgrading:** before [api-components-bundle#295](https://github.com/components-web-app/api-components-bundle/pull/295), the bundle's mappings replaced API Platform's defaults in every application, so these errors returned `500`. A client that handled them as server errors now gets a `4xx`.
::
