...
**If real-time updates stop but saves still work, check the API log for Mercure errors.** When the hub can't be reached, the bundle logs the failed publish at error level, with its topics and the exception, and the save still succeeds. Other open pages just stop receiving updates. Before [api-components-bundle#284](https://github.com/components-web-app/api-components-bundle/pull/284), the same failure turned an already-saved write into a 500.
::
