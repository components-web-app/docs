...
const currentStyleName = computed(() => {
  ...
  return getCurrentStyleName(resource.value.data) ?? 'Default'
})
...
**`styles.multiple`** — `false` means the admin picks one style. `true` allows multiple styles to be combined. Don't add a `'Default'` entry yourself: the single-style picker already offers **Default** (no style), and `getCurrentStyleName` returns `undefined` when that is selected.
...
IMAGE: The manager panel's UI tab showing the style picker — the named options from `styles.classes` as a single-select list, with one applied
::

::callout{icon="i-heroicons-video-camera"}
VIDEO: Selecting a NavigationLink in edit mode, opening the **UI** tab, switching the **Style:** select from Default to Filled Button and then Outlined Button, with the link restyling live on the page each time, then back to Default
::
...
::callout{icon="i-heroicons-video-camera"}
VIDEO: On the **UI** tab, switching the **UI:** select from Default to YouTube: the plain text link is swapped for the YouTube icon on the page, and the **Style:** select disappears because the `ui/` variant declares no `styles`
::

### When to use `ui/` vs `styles.classes`

::callout{icon="i-heroicons-photo"}
IMAGE: The same component rendered twice side by side — the default template and its `ui/` variant — to show how far a UI template can diverge from a style class
::
