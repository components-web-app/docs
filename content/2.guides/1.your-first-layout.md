...
The `v-if` matters. `layoutIri` is `undefined` until the layout resource resolves, but `location` is a required `string` — without the guard you get a type error and a "location is not a current resource" warning box on first render.
::
