...
IMAGE: The `npx cwa make:component` prompts in a terminal — name, type and API behaviour questions — ending with the generated file path and the `php bin/console make:api-component` command to copy
::
...
See [`make:api-component`](/api/console-commands#make-api-component) for all available flags.
...
::callout{icon="i-heroicons-photo"}
IMAGE: The manager panel open on a Title component, showing the "Title" admin tab with its text input
::
...
VIDEO: The full loop — entering edit mode, adding a Title from the Add Component dialog, typing in the admin tab and watching the heading update live, then clicking Publish
::
