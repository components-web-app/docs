...
| `allowed-components` | `string[] \| null` | No | — | Collection endpoints of the component types admins may add to this group (e.g. `['/component/hero_banners', '/component/text_blocks']`). All CWA component endpoints are prefixed with `/component/`. When passed, the value is synced to the `ComponentGroup` entity in the API whenever a user is signed in, and an explicit `null` clears the list (allowing all types). When omitted, the group's stored list is left alone, so a list set by fixtures or the REST API is kept. |

## Basic Usage

Every layout, page template, and component that has editable content regions uses `<CwaComponentGroup>`. In a page template, pass its own `iri` prop as `location`:
...
- **Page template:** `props.iri`.
- **Component:** `publishedIri` from `useCwaComponent()`, **not** `iri`. While an admin is editing, a component that has a draft is rendered under its draft IRI. The group is looked up as `{reference}_{location}`, so `:location="iri"` would look for a group keyed on the draft, which doesn't exist, and the group's contents disappear from the page. This typically happens as soon as an edit, such as uploading a new image, creates a draft.
...
<template>
  <div class="grid grid-cols-2 gap-8">
    <div>
      <CwaComponentGroup reference="left" :location="publishedIri" />
    </div>
    <div>
      <CwaComponentGroup reference="right" :location="publishedIri" />
    </div>
  </div>
</template>
...
// publishedIri stays the same while a draft is being edited, so the groups keep their contents
...
**Via the Vue prop** — pass collection endpoint paths directly in the template. The module syncs this to the API entity whenever a user is signed in (the API enforces who may actually write), creating or updating the group as needed:
...
::callout{icon="i-heroicons-information-circle"}
The prop only overwrites the stored list when you pass it. To manage a group's list with fixtures or the REST API instead, leave the prop out of the template.
::
