...
// nuxt.config.ts
cwa: {
  pagesDepth: 3,  // generates nested routes for depths 0–3, i.e. up to /a/b/c
  pages: {
...
`pagesDepth` controls how many nested route levels the module registers with Nuxt's router (default `4`). A route is generated at depth 0 *and* at every depth below it, so `pagesDepth: 3` covers URLs up to three segments deep (`/blog/tech/my-post`).
