...
Without `<CwaPage />` in the parent template, the child page has no position of its own: CWA appends it after the parent template's output (see [Auto-Fallback](#auto-fallback)).
...
// nuxt.config.ts
cwa: {
  pagesDepth: 3,  // registers a route for each depth 0–3, i.e. up to /a/b/c
  pages: {
...
`pagesDepth` controls how many route levels the module registers with Nuxt's router (default `4`). It registers one route for each depth from 0 to `pagesDepth`, so `pagesDepth: 3` covers URLs up to three segments deep (`/blog/tech/my-post`).

It is a build-time setting only — it doesn't affect what CWA fetches. Rendering depth itself comes from the resource manifest.
...
A link to a new page opens it at the top. Going Back returns to where you were, and a link to another page's `#section` scrolls to that section. CWA fetches a page's content after the page mounts, so for these two cases the module waits until the page is tall enough, or the target element exists. It waits for at most one second.

To keep the scroll position on one of your own Nuxt pages, set `scrollToTop: false` in its `definePageMeta`. An app that has its own `app/router.options.ts` with a `scrollBehavior` replaces the module's, and loses the wait.

::callout{icon="i-heroicons-information-circle"}
**Requires `@cwa/nuxt-edge` `0.0.0-29836672.c74ef41` or later.** Before it, going Back could land near the top, and a new page could open at the last page's scroll position.
::
