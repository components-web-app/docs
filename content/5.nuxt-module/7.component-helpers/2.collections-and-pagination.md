...
```ts
const { model: searchModel } = useQueryBoundModel('search', { delay: 250 })
```

The parameter names must match what the API resource declares. With the `search` and `order[:property]` parameters from [Dynamic Pages → Searching and Sorting](/api/dynamic-pages#searching-and-sorting), use `search` for the text box. A per-field name such as `title` is ignored, so the list would never narrow.
...
The `Collection` entity comes from the bundle, but its manager tab does not — neither the bundle nor the module ships one. Add it as an app-side admin tab, as the template does at `app/cwa/components/Collection/admin/CollectionTab.vue`, binding the entity's three fields with `useCwaResourceModel`:

1. `resourceIri` — which collection endpoint to list (the template offers a fixed select of endpoints, e.g. `/page_data/blog_article_datas`)
2. `perPage` — how many items per page
3. `defaultQueryParameters` — default sort and filter values
