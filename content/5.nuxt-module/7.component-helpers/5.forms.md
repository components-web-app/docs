...
`vars.choices` is an array of `{ label, value }` objects from the API.
...
Symfony's `CheckboxType` submits `'1'` when checked and `''` when unchecked. `useCwaFormInput` initialises `value` from `vars.checked` (`'1'` or `''`), so read the boolean state from `!!field.value.value` rather than `vars.checked`:
...
New collection entries have no `vars` in the store until the first validation or submission response arrives. Use hardcoded label fallbacks (e.g. `field.vars.value?.label || 'Name'`) until the API returns `formView` data.
::
