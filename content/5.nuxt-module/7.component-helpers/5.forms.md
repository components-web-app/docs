...
Symfony's `CheckboxType` always sends `vars.value = '1'`; the checked boolean state comes from `vars.checked`. Use a computed to bridge:

```vue
<script setup lang="ts">
const agree = useCwaFormInput(iriRef, 'form[agreeToTerms]')
const isChecked = computed({
  get: () => agree.vars.value?.checked ?? false,
  set: (v: boolean) => {
