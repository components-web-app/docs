...
When the field has a placeholder, Symfony sends it **twice**: as `vars.placeholder` (the label text) and as an extra `{ value: '', label: '…' }` entry in `vars.choices`. Filter the empty-value entry out of `:items` and pass `vars.placeholder` to `:placeholder` instead — left in the list it renders as a real, selectable option, and Symfony 422s on the empty value.
...
>
  <USelectMenu
    ...
    :items="(tags.vars.value?.choices || []).filter(c => c.value !== '')"
    :placeholder="tags.vars.value?.placeholder"
    :multiple="true"
    value-key="value"
    label-key="label"
...
Same placeholder handling as the single select above. The **expanded** variants (radio and checkbox group) take no placeholder — leave their `:items` unfiltered.
...
// Spinner while the validation PATCH is in flight, tick once the entry is valid
  ...
  <UFormField :error="field.displayErrors.value ? field.errors.value[0] : undefined">
    <div class="flex items-center gap-2">
      <UInput
        ...
        :ui="{ trailingIcon: trailingIconClass }"
        @blur="field.onBlur"
        @input="field.onInput"
      />
