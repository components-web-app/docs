...
The API exposes a `realtime_validate_disabled` form option (set on `UserLoginType` and `PasswordUpdateType`) that surfaces in the root form vars, but the module does not currently read it — live validation runs regardless. Skip `@input`/`onInput` on a field if you don't want it.
::
