...
Each style's value is a **class string** (a `string[]` is also accepted and joined). The style names appear in the admin "Style" selector. The selected classes are automatically applied to the component's root element — no `:class` binding needed.
