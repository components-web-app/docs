...
| `displayErrors` | `ComputedRef<boolean>` | Gated on both fields having been blurred — see below |
| `onBlur` | `() => void` | Call on the input's `blur` event |
| `onInput` | `() => void` | Debounced; triggers cross-field validation |
| `validate` | `(extraData?) => Promise<void>` | Explicit validate — PATCHes to the form's `/submit` endpoint |

## Error display gate

`displayErrors` is `false` until **both** `first` and `second` have been blurred at least once — *or* the form has been submitted and failed, *or* the pair was previously valid and has since become invalid. This stops a mismatch error appearing on the second field the moment the user leaves the first, without hiding errors after a failed submit. It is also suppressed while a validation request is in flight.
