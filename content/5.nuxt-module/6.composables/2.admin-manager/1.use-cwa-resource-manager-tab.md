...
`disabled` is declared as `boolean` in `CwaResourceManagerTabOptions`, so passing a `ComputedRef` — as above — works at runtime but is a TypeScript error. Until the module widens the type, either cast it or compute a plain boolean.
::
