...
The same applies to programmatic navigation: use `navigateTo(getInternalResourceLink(iri))`, never `navigateTo(iri)`. The `cwaPage0` param exists only on the `/_cwa/:cwaPage0` route, so a bare IRI string instead matches one of the module's catch-alls (`/`, `/:cwaPage1`, …) with no `cwaPage0` at all. The fetcher then requests `/_/routes/` plus the whole IRI — a primary fetch that can only 404, taking over the screen while the URL stays put.
::
