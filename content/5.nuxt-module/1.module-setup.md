...
Tagged releases are published to npm as `@cwa/nuxt`, starting with `2.0.0-alpha.1`. This is what the template app depends on:

```bash
pnpm add @cwa/nuxt@alpha
```
...
Tagged releases, from `2.0.0-alpha.1` on, are listed in the module's [`CHANGELOG.md`](https://github.com/components-web-app/cwa-nuxt-module/blob/dev/CHANGELOG.md) and on its [GitHub releases](https://github.com/components-web-app/cwa-nuxt-module/releases), which use the same notes. Changes on `dev` that aren't in a release yet are under **Unreleased**. For the API bundle's changes, see [Bundle Setup](/api/bundle-setup#installation).
