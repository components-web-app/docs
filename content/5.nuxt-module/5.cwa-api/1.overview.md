...
`page`, `pageData` and `displayPage` are `ComputedRef | undefined` — the getter itself returns `undefined` while the corresponding IRI is unresolved. Optional-chain the getter (`cwa.resources.page?.value`), not just `.value`.
::
