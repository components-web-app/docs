...
One exception worth knowing before you write `.value` anywhere: `currentIds`, `currentResources`, `getComponentGroupByReference()`, `isIriPublishableEquivalent()` and `findAllPublishableIris()` return plain values, not refs — no `.value`.
::
...
cwa.resources.pageData      // ComputedRef<CwaCurrentResourceInterface | undefined> — value is set on data pages only
cwa.resources.pageDataIri   // ComputedRef<string | undefined>

cwa.resources.displayPage   // ComputedRef — the "identity" page shown in the admin (pageData for data pages, page otherwise)
cwa.resources.displayPageIri
```

`page`, `pageData` and `displayPage` always return a `ComputedRef`, like `layout` — read `.value` directly. The *value* is `undefined` until the corresponding IRI resolves, so guard the data you read from it, not the getter.
...
Use these when you need to know **what renders at a specific nesting level**. They read `manifest.irisByDepth[n]`. At depth 0, `pageIriAtDepth()` and `pageAtDepth()` fall back to the flat fetch path; the pageData methods do not, so without a manifest they return `undefined` even on a data page — use `pageDataIri` / `pageData` there. At depth 1 and beyond a manifest is required.
...
```ts
cwa.resources.isLoading         // ComputedRef<boolean> — true while any fetch is resolving or resources are still loading
