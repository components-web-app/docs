...
Use these when you need to know **what renders at a specific nesting level**. They read `manifest.irisByDepth[n]`. At depth 0, all four methods fall back to the page and page data loaded by the primary fetch, so they still resolve when the manifest is missing. At depth 1 and beyond a manifest is required.
