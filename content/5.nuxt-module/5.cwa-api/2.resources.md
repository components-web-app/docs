...
One exception worth knowing before you write `.value` anywhere: `currentIds`, `currentResources`, `isIriPublishableEquivalent()` and `findAllPublishableIris()` return plain values, not refs — no `.value`.
::
