...
The `@cwa/nuxt` module ships as a Nuxt layer. A layer is a self-contained Nuxt application whose pages, layouts, components, and plugins are merged into your project automatically. You add the layer with `extends: ['./node_modules/@cwa/nuxt/dist/layer']` in `nuxt.config.ts`, and the layer registers the module for you — not the other way round. Registering the module directly under `modules` leaves out the layer's auth pages, admin panel, root layout and middleware.
