...
| `/_cwa/routes` | Manage routes — path, redirects, and visibility (live, scheduled or not live; see [Visibility](/core-concepts/admin-panel#visibility)) |
...
| `/_cwa/orphaned` | The last orphaned resource report: view and delete orphaned components, positions and groups. Opened from site settings, which can also start a scan. See [Orphaned Resources](/core-concepts/admin-panel#orphaned-resources). Needs a module build after 2.0.0-alpha.2 |
| `/_cwa/users` | List and manage user accounts |
| `/_cwa/users/[iri]` | Edit an individual user. A pending email change shows **Resend email verification** (disabled with a countdown while throttled) and **Cancel change**, which clears the pending address so its emailed link stops working |

All `/_cwa` routes are client-only (SSR disabled). Access is guarded inside the admin index page itself: non-admins are redirected to `/` on mount, and signing out while on an admin route sends you back to `/`. To guard your own admin-only pages, use the layer's `cwa-admin` route middleware — see [Protecting Your Own Pages](/nuxt-module/cwa-layer/auth-pages#protecting-your-own-pages).
