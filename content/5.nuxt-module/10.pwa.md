...
[Page caching](/nuxt-module/page-caching) is on by default. A cached page's lifetime is the lowest of `pageCache.sharedMaxAge` (one hour by default) and the `s-maxage` of every API response the page used. In production that gives one hour. In development the API's 60 seconds is lower, so pages expire after a minute. Keep this in mind when you test purging locally: a page that looks invalidated may simply have expired.
