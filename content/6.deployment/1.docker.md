...
**Tracking parameters are stripped before the cache**: The Caddyfile's `uri query { … }` block removes `utm_*`, `gclid`, `fbclid` and other tracking parameters, so they don't split the cache. Nuxt and php never receive them. See [Tracking parameters](/nuxt-module/page-caching#tracking-parameters) for the list and how to change it.
