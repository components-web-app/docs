...
**A page that uses a draft component is never cached**: When an anonymous visitor loads the page, the request for the unpublished component returns a 404. The 404 response has `Cache-Control: no-cache, private`. The module combines the cache headers of every API response it fetched, so that one private response makes the whole page `private, no-store`. Other pages are still cached. If one page is never cached, check whether it uses an unpublished component before you change the cache configuration.
