...
The pwa container gets the same TCP liveness probe on port 3000 and a TCP startup probe that checks every 5 seconds rather than 10, with `/_cwa/healthcheck` for readiness. That path is not cached, so the default timeout is fine there.
