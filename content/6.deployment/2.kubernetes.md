...
The pwa container gets the same TCP startup and liveness probes on port 3000, with `/_cwa/healthcheck` for readiness. That path is not cached, so the default timeout is fine there.
