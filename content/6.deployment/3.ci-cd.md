...
- **Up to bundle `2.0.0-alpha.4`**, which the template's `composer.lock` currently pins, the scaffold stops at the first page route that already exists. Nothing is written, and the step reports a failure, which is allowed so it does not fail the deploy.
- **From the next bundle release** (api-components-bundle#335, on `main`), appending creates only what is missing and keeps existing content. See [Appending to an Existing Site](/api/data-fixtures#appending-to-an-existing-site).
