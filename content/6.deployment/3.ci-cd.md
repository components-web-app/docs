...
Four workflows drive your app, in `.github/workflows/`:
...
A fifth file, `publish-create-cwa.yml`, is template-repo tooling that publishes the scaffolding CLI. It is copied into generated projects but does nothing useful there — safe to delete.
...
A manual job that runs [sitespeed.io](https://www.sitespeed.io/) against the environment URL saved by the preceding deploy job (`environment_url.txt`). Reads URLs from `.gitlab-urls.txt` if present, otherwise tests the environment root. Results upload as GitLab artifacts.
