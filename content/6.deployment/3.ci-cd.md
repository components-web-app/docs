...
By default it audits the first five pages of `/sitemap.xml`, on mobile, three runs each.
...
| `PERFORMANCE_AUDIT_MAX_PAGES` | `5` | How many sitemap pages to audit when `PERFORMANCE_AUDIT_URLS` is unset |
...
| `PERFORMANCE_AUDIT_RUNS` | `3` | Runs per page. Budgets use the median run |
| `PERFORMANCE_AUDIT_CONFIG` | `bin/devops/lighthouserc.json` | The Lighthouse CI config that holds the budgets |
...
The reports go in `performance-report/`: Lighthouse's HTML and JSON for each page and form factor, the budget results, and a `summary.md` table. The table is also printed at the end of the log.
...
Screenshot: the summary of a GitHub **Performance audit** run, showing the per-page table (page, form factor, score, LCP, CLS, TBT, FCP, Speed Index, page weight) and the "Performance audit" warning annotation from a missed budget.
::
