...
| `load_fixtures [track]` | Loads fixtures into the php pod. Manual jobs only on GitLab (see `ENABLE_DATABASE_FIXTURES`) |
...
When `ENABLE_DATABASE_FIXTURES` is `"true"` (a GitLab CI/CD variable), a **load fixtures** job appears for each environment (review, staging, production) after the deploy job completes. It does not run automatically — trigger it manually from the pipeline UI when you want to seed the database.

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
The GitHub Actions workflows do not read `ENABLE_DATABASE_FIXTURES`. They call `load_fixtures` straight after `deploy` in the review and staging jobs (`ci.yml`) and in the production job (`production.yml`), so fixtures load on every GitHub deploy. `load_fixtures` runs `doctrine:fixtures:load` without `--append`, which empties the database first. Remove those calls before you deploy anything with real data from GitHub.
::
