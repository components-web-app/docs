...
- **Empty `MAILER_EMAIL` crashes the scan.** The config reads `%env(default::MAILER_EMAIL)%`, which turns an empty value into `null`, so `scan-orphaned` fails with a `TypeError` before it scans. Change it to `%env(string:default::MAILER_EMAIL)%` in `api/config/packages/silverback_api_components.yaml`, or set `MAILER_EMAIL`.
