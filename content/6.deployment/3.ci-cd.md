...
The email goes to `MAILER_EMAIL`, the same address the site sends from (`orphaned_resources.notify.recipients` in `api/config/packages/silverback_api_components.yaml`). Use a bare address such as `website@example.com`. A `Name <address>` value is refused until [api-components-bundle#361](https://github.com/components-web-app/api-components-bundle/issues/361) is fixed; see [Emailing when orphans change](/api/console-commands#emailing-when-orphans-change).

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**Set `MAILER_EMAIL` on production.** With bundle `2.0.0-alpha.6`, an empty `MAILER_EMAIL` doesn't just skip the email: the template's `%env(default::MAILER_EMAIL)%` turns it into `null`, and `scan-orphaned` fails with a `TypeError` before it scans, so the report isn't refreshed either.
::

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**GitHub Actions doesn't pass these variables.** The deploy jobs in `ci.yml` and `production.yml` name none of `ORPHAN_SCAN`, `ORPHAN_SCAN_SCHEDULE`, `ORPHAN_SCAN_TIMEZONE`, `MAILER_EMAIL` or `MAILER_DSN` in their `env:`. A GitHub production deploy still gets the CronJob with the defaults above, but setting the variables in GitHub has no effect until you add them to each deploy job's `env:` block.
::
