...
1. Copy `api/.env` to `api/.env.local` - this will be your private environment variables and will not be pushed to your git repository.
   ...
   cp .env .env.local
   ```
1. Then find and change `JWT_PASSPHRASE` in your `api/.env.local` - this should always be a secret and you can use real environment variables in production. Note down the passphrase. You could use this command to create the secret.
   ```bash [Terminal]
