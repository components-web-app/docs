...
description: The seven resource types that form the content graph behind every CWA page — and how they chain together.
badge:
  label: Draft
  color: amber
---
...
- Rendered by its type: a `Title` resource renders `app/cwa/components/Title/Title.vue`. Its `uiComponent` stays empty unless an admin picks a `ui/` variant
