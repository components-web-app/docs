...
**For signed-in users with publish rights**: the API returns the draft version. This is decided from the auth cookie and the user's role — not from whether edit mode is toggled on — so an admin sees drafts on every page view, not just while editing. Add `?published=true` to a request to force the published version. Edits via `useCwaResourceModel` (the debounced `v-model` binding) patch the draft.
...
**When an admin schedules a publish**: the draft's `publishedAt` is set to a future time from the **Publish** tab, and the draft replaces the live version at that time. See [The Admin Panel → Scheduling a Publish](/core-concepts/admin-panel#scheduling-a-publish).

::callout{icon="i-heroicons-photo"}
IMAGE: A component in the manager panel showing an unpublished draft state alongside the Publish action
::
...
## Scheduling a Whole Page

Draft and publish work per component. To hold back a **whole page**, or take one offline, use its route's go-live date instead. A route can be **Live**, **Scheduled** for a date and time, or **Not live**; until it is live, visitors get a 404 and the page stays out of the sitemap, while admins can still preview and edit it. A scheduled parent holds back every page nested beneath it.

Set it from the route's **Visibility** setting in the admin — see [The Admin Panel → Visibility](/core-concepts/admin-panel#visibility) — or through the API, covered in [Scheduling and Taking Routes Offline](/api/dynamic-pages#scheduling-and-taking-routes-offline).
