...
::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
**Upgrading:** this needs `@cwa/nuxt` `2.0.0-alpha.3` or later. It also needs an api-components-bundle release containing [#353](https://github.com/components-web-app/api-components-bundle/pull/353), which added the delete endpoint and is after `2.0.0-alpha.5`. Development builds before it sent one `DELETE` per row, which left the draft of an orphaned component behind as a component of its own.
::
