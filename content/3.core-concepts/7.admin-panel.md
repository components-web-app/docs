...
When the last scan found anything, a notice at the top of settings reads "Orphaned resources discovered", with the count and a **Review now** button. Opening settings only reads the stored report. It never starts a scan, so the report is only as fresh as the last time someone clicked a scan button.
...
Each section with rows has **Delete all**, and **Delete everything** deletes every row on the page, components first, then positions, then groups. Every delete asks you to confirm first. A resource that has already gone counts as deleted.
...
The admin sends one `DELETE` per row, to the resource's IRI (a component's published version). It doesn't use the API's bulk [`POST /_/orphaned_resources/delete`](/api/console-commands#deleting-orphans) yet. A `DELETE` sent to a published component keeps its draft, so deleting an orphaned component that has a draft leaves the draft behind as a component of its own. It is listed the next time you scan; delete it then.

After bundle `2.0.0-alpha.5`, every component a delete removes along the way takes its own groups and its unused draft with it, so those no longer turn up in the next scan. Only the draft of the component you deleted yourself is kept.

Deleting removes the row but doesn't scan again. The page says the report reflects the scan shown at the top; anything else that has changed since appears when you click **Scan again**.
