...
- `uiClassNames` — the CSS classes currently selected for this layout, stored as an array (one entry per selected style). The list an admin picks from is declared in `nuxt.config` under `cwa.layouts.<Name>.classes`; `useCwaLayout()` applies whatever is stored here to the root element
...
The `reference` is a stable name for this region. The `location` is the layout's own IRI (available from `$cwa.resources.layoutIri`). The `allowed-components` restricts what types can be added to this group, by component name.
...
In the admin, template pages are shown in **yellow** and PageData records in **green** (**orange** until they have a route) — making it easy to see at a glance what is shared structure versus individual content.

### What the Public Can See

A page, and everything on it, is public only when a **live route** leads to it. That route can be the page's own, or one on a page nested beneath it: a parent page with no URL of its own still renders for visitors on a child's URL. A page that nothing routes to, such as a draft page or an unused template, is visible only to admins.

::callout{icon="i-heroicons-information-circle"}
The full rule, including the upgrade note for pages with no route, is in [Users & Security → Public Access Follows Routes](/api/users-and-security#public-access-follows-routes).
::
...
IMAGE: The `/_cwa/layouts` admin listing with several layouts, and a page's settings showing which layout it is assigned
::
