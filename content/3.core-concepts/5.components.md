...
The module passes the component's IRI as a prop. `useCwaComponent` reads the resource from the module's store — the module has already fetched it — and stays reactive to updates. `defineExpose(exposeMeta)` wires the component into the admin manager.
...
VIDEO: Typing in a component's admin tab and the rendered component updating live on the page behind the manager panel, with no save step
::
...
|---------|---------|---------------|
| Text field | Title, TextBlock | `useCwaComponent` + `useCwaResourceModel` |
| File upload | Image, HeroSection | `useCwaComponent` + `withFile()` + `useCwaResourceUpload` |
| Rich text | HtmlContent | `useCwaComponent` + TipTap editor in admin |
...
Each of these is covered in detail in the [Guides](/guides/your-first-layout) section with step-by-step instructions.
